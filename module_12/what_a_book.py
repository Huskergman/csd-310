...
def show_menu():
    ...
    try:
        choice = int(input('      <Ex. enter: 1 to showcase books>: '))

        return choice
    except ValueError:
        print("\n  Invalid number, closing program...\n")
...
try:
    ...
    while user_selection != 4:
        ...
        # if the user selects option 3, call the validate_user method to validate the user_id call the show_account_menu() to show the account settings menu
        if user_selection == 3:
            my_user_id = validate_user()
            account_option = show_account_menu()

            # call when account option doesn't equal 3
            while account_option != 3:

                # if the use selects option 1, call the show_wishlist() method to show the users wishlist
                if account_option == 1:
                    show_wishlist(cursor, my_user_id)

                # if the user selects option 2, call the show_books_to_add function to list books not in the users wishlist
                if account_option == 2:

                    # shows books not under users wishlist
                    show_books_to_add(cursor, my_user_id)

                    # get book from the user 
                    book_id = int(input("\n        Enter the id of the book you want to add to your wishlist: "))
                    
                    # add the requested book to the users wishlist
                ...
                # if the selected option does not fit the range, display an invalid user selection 
        ...
        # if the user selection does not meet the requirements, display an invalid user selection
